...
def getCompanyInfo(url_all):
    ...
    compInfos.columns = var
        ...
        try:
            try:
                shareholder = html.xpath('//div[@id="_container_holder"]//a[@class="link-click"]//text()')
            except:
                shareholder = "NaN"
            try:
                share_pct = html.xpath('//div[@id="_container_holder"]//td[3]/div/div/span[@class]//text()')
            except:
                share_pct = "NaN"
                
            try:
                capital_con = html.xpath('//div[@id="_container_holder"]//td[4]/div/span//text()')
            except:
                capital_con = "NaN"        
            shareholderInfo = [shareholder,share_pct,capital_con]
            shareholderInfos = shareholderInfos.append(pd.DataFrame(shareholderInfo).T)
        except:
            pass
    var_shareholder = ['股东名称', '持股比例', '认缴出资额'] 
    shareholderInfos.columns = var_shareholder
        
        
        
    return compInfos,shareholderInfos
